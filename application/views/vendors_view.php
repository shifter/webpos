...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
        <div class="box">
            <div class="box-header" style="">
              <button class="btn btn-primary" id="btn_new"><i class="fa fa-plus-circle" aria-hidden="true"></i> New Vendors</button>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
    ...
    <!--modal-->
        <div id="modal_vendors" class="modal fade"  tabindex="-1" role="dialog" aria-hidden="true">
...
<?php echo $_rights; ?>
    ...
    ?>
    <?php
        if($this->session->vendors_update == 0 && $this->session->vendors_delete == 0){
    ?>
            v = false;
    <?php
        } 
    ...
    var btn_edit = "";
    var btn_trash = "";
    var _cboVendors; var _cboBrand; var _cboVendors; var _cboVendor;
            ...
            "columns": [
                ...
                { targets:[2],data: "vendor_desc" },
                {
                    ...
                    render: function (data, type, full, meta){
                     <?php
                        if($this->session->vendors_update == 1){
                    ?>      
                    ...
                    <?php
                        ...
                        if($this->session->vendors_delete == 1){
                    ?>      
    ...
    $('#btn_create').click(function(){
            if(validateRequiredFields($('#frm_vendors'))){
                if(_txnMode==="new"){
                    //alert("aw");
                    createVendors().done(function(response){
                        showNotification(response);
                        dt.row.add(response.row_added[0]).draw();
                        clearFields($('#frm_vendors'))
                    }).always(function(){
                        $('#modal_vendors').modal('hide');
                        $.unblockUI();
                    });
                    return;
                }
                if(_txnMode==="edit"){
                    //alert("update");
                    updateVendors().done(function(response){
                        showNotification(response);
    ...
    var delete_notif=function(type){
            swal({
                    ...
                    if (isConfirm) {
                            removeVendors().done(function(response){
                            showNotification(response);
