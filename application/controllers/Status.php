...
class Status extends CORE_Controller
{
    ...
    function transaction($txn = null) {
      function replaceCharsInNumber($num, $chars) {
        ...
        switch ($txn) {
            case 'list':
                $m_status = $this->Status_model;
                $response['data']=$m_status->get_list(array('status.is_deleted'=>FALSE));
                echo json_encode($response);
            break;

            case 'create':
                $m_status = $this->Status_model;
                $m_status->status_name = $this->input->post('status_name', TRUE);
                $m_status->save();

                $status_id = $m_status->last_insert_id();

                $format = "000000";
                $temp = replaceCharsInNumber($format, $status_id); //5069xxx
                $ecode = $temp .'-'. $today = date("Y");

                $m_status->status_code=$ecode;

                $m_status->modify($status_id);

                $response['title'] = 'Success!';
                $response['stat'] = 'success';
                $response['msg'] = 'Status information successfully created.';
                $response['row_added'] = $m_status->get_list($status_id);
                echo json_encode($response);
            ...
            case 'update':
                ...
                $response['row_updated']=$m_status->get_list($status_id);
                echo json_encode($response);
